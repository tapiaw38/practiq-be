...
func (r *repository) Upsert(ctx context.Context, n domain.Notification) error {
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO notifications (user_id, type, title, body, resource_type, resource_id, scheduled_at, school_id)
		...
		DO UPDATE SET
			...
			read_at = NULL,
			created_at = NOW()
	`, n.UserID, n.Type, n.Title, n.Body, nullable(n.ResourceType), nullable(n.ResourceID), n.ScheduledAt, tenant.SchoolID(ctx))
	return err
}
...
func (r *repository) MarkAllRead(ctx context.Context, userID string) error {
	_, err := r.db.ExecContext(ctx, `
		UPDATE notifications SET read_at = NOW()
		WHERE user_id = $1 AND read_at IS NULL AND ($2 = '' OR school_id = NULLIF($2,'')::uuid)
...
func (r *repository) Delete(ctx context.Context, id, userID string) (bool, error) {
	result, err := r.db.ExecContext(ctx, `
		DELETE FROM notifications WHERE id = $1 AND user_id = $2 AND ($3 = '' OR school_id = NULLIF($3,'')::uuid)
	`, id, userID, tenant.SchoolID(ctx))
	if err != nil {
		return false, err
	}
...
func (r *repository) DeleteByResource(ctx context.Context, notificationType, resourceID string) error {
	_, err := r.db.ExecContext(ctx, `
		DELETE FROM notifications WHERE type = $1 AND resource_id = $2 AND ($3 = '' OR school_id = NULLIF($3,'')::uuid)
	`, notificationType, resourceID, tenant.SchoolID(ctx))
	return err
}
