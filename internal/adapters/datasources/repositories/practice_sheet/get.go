...
func (r *repository) Get(ctx context.Context, id string) (*domain.PracticeSheet, error) {
	...
	err := row.Scan(&ps.ID, &ps.CourseID, &ps.TopicID, &ps.StrategyID, &ps.Title, &ps.Level, &ps.SheetType, &ps.TestStyle, &ps.ScheduledAt, &ps.AvailableUntil, &ps.MaxAttempts, &ps.TimeLimitMinutes, &ps.CreatedBy, &ps.CreatedAt)
