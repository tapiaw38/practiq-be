...
func (r *repository) List(ctx context.Context, filter ListFilter) ([]domain.PracticeSheet, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ps.ID, &ps.CourseID, &ps.TopicID, &ps.StrategyID, &ps.Title, &ps.Level, &ps.SheetType, &ps.TestStyle, &ps.ScheduledAt, &ps.AvailableUntil, &ps.CreatedBy, &ps.CreatedAt); err != nil {
			return nil, err
		}
