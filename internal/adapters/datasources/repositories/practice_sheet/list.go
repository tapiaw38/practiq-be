...
func (r *repository) List(ctx context.Context, filter ListFilter) ([]domain.PracticeSheet, error) {
	query := `
		SELECT ps.id, ps.course_id, COALESCE(ps.topic_id::text,''), COALESCE(ps.strategy_id::text,''), ps.title, ps.level, ps.sheet_type, ps.test_style, ps.scheduled_at, ps.available_until, ps.max_attempts, ps.time_limit_minutes, ps.created_by, ps.created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ps.ID, &ps.CourseID, &ps.TopicID, &ps.StrategyID, &ps.Title, &ps.Level, &ps.SheetType, &ps.TestStyle, &ps.ScheduledAt, &ps.AvailableUntil, &ps.MaxAttempts, &ps.TimeLimitMinutes, &ps.CreatedBy, &ps.CreatedAt); err != nil {
			return nil, err
		}
