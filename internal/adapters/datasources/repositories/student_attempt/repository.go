...
func (r *repository) Create(ctx context.Context, a domain.StudentAttempt) (string, error) {
	query := `
		INSERT INTO student_attempts (student_id, exercise_id, practice_sheet_id, answer_text, is_correct, score, time_spent_seconds, hints_used)
		VALUES ($1, $2, NULLIF($3,'')::uuid, $4, $5, $6, $7, $8)
		RETURNING id
	`
	var id string
	err := r.db.QueryRowContext(ctx, query, a.StudentID, a.ExerciseID, a.PracticeSheetID, a.AnswerText, a.IsCorrect, a.Score, a.TimeSpentSecs, a.HintsUsed).Scan(&id)
	return id, err
}
...
func (r *repository) SaveCanvasWork(ctx context.Context, attemptID, imageData string) error {
	query := `
		...
		VALUES ($1::uuid, $2)
		ON CONFLICT (attempt_id) DO UPDATE SET image_url = EXCLUDED.image_url
	`
	_, err := r.db.ExecContext(ctx, query, attemptID, imageData)
	return err
}

func (r *repository) ListBySheet(ctx context.Context, studentID, sheetID string) ([]domain.StudentAttempt, error) {
	query := `
		SELECT id, student_id, exercise_id, COALESCE(practice_sheet_id::text,''), answer_text, is_correct, score, time_spent_seconds, hints_used, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.StudentID, &a.ExerciseID, &a.PracticeSheetID, &a.AnswerText, &a.IsCorrect, &a.Score, &a.TimeSpentSecs, &a.HintsUsed, &a.CreatedAt); err != nil {
			return nil, err
		}
