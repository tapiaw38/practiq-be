...
func (r *repository) UpsertSubmission(ctx context.Context, s domain.NotebookSubmission) error {
	result, err := r.db.ExecContext(ctx, `
		INSERT INTO notebook_submissions (page_id, student_id, canvas_data, answer_text, ai_recognized_text, ai_is_correct, ai_feedback, ai_reviewed_at, needs_teacher_review, school_id)
		SELECT $1, $2, $3, $4, $5, $6, $7, $8, $9, NULLIF($10,'')::uuid
		WHERE EXISTS (
			SELECT 1 FROM notebook_pages np
			JOIN notebooks n ON n.id = np.notebook_id
			WHERE np.id = $1 AND n.deleted_at IS NULL AND ($10 = '' OR n.school_id = NULLIF($10,'')::uuid)
		)
		...
		DO UPDATE SET
			...
			updated_at = NOW()
	`, s.PageID, s.StudentID, s.CanvasData, s.AnswerText, s.AIRecognizedText, s.AIIsCorrect, s.AIFeedback, s.AIReviewedAt, s.NeedsTeacherReview, tenant.SchoolID(ctx))
	if err != nil {
		return err
	}
